...
protocol VideoToGifViewControllerDelegate {
    func videoDidConvert(path: String)
}

class VideoToGifViewController: NSViewController {
    ...
    var delegate:VideoToGifViewControllerDelegate!
    ...
    private func updateVideoState() {
        ...
        case .converted(let destinationURL):
            if let originalVideoURL = videoURL, let gifURL = gifURL, let destinationURL = destinationURL {
                ...
                self.delegate.videoDidConvert(path: finalGIFURL.path)
            }
    ...
    private func convertToGIF() {
        ...
        if case .selected(let url) = videoState {
            ...
            NSGIF.createGIFfromURL(url, withFrameCount: Int32(frameRateSlider!.integerValue), delayTime: Float(Int32(frameRateSlider!.integerValue)/100), loopCount: 0) { (url) in
