...
class FrameStore {
  ...
  init(data: Data, size: CGSize, contentMode: NSImageScaling, framePreloadCount: Int, loopCount: Int) {
  ...
  func frame(at index: Int) -> NSData? {
    return animatedFrames[safe: index]?.image?.uncompressed(using: .lz4) as NSData?
  }
...
private extension FrameStore {
  ...
  func loadFrame(at index: Int) -> NSData {
    let imageRef = CGImageSourceCreateImageAtIndex(imageSource, index, nil)
    
    let image = NSImage(cgImage: imageRef!, size: CGSize(width: 0, height: 0))
    let scaledImage: NSImage?

//    if shouldResizeFrames {
//      switch self.contentMode {
//      case .scaleProportionallyUpOrDown: scaledImage = image.constrained(by: size)
//      case .scaleAxesIndependently: scaledImage = image.filling(size: size)
...
//    }
    ...
    return scaledImage!.tiffRepresentation! as NSData
  }
